...
```{r}
library(ggplot2)
library(FNN) #very fast implementation of nearest neighbors
...
X <- scale(data.matrix(diamonds[, x])) #data matrix turns factors into integers, scale for std-dev = 1

# The 10'000th observation
diamonds[10000, c("price", "carat", x)]

# Its prediction
knn.reg(X, test = X[10000, ], k = 5, y = diamonds[[y]]) #prediction for the 10000 diamond. the result has the 10000 diamond in it, which is a clear sign of overfitting. the dataset also includes the same diamonds 
...
cat("Validation RMSE:", RMSE(y_valid, pred_valid)) #might be too optimistic, if validation set has 1 and train set has 2 of the same diamonds
```
...
ggplot(aes(x = k, y = RMSE, group = Data, color = Data)) +
  geom_point() +
  geom_line()
# overfitting for smaller values of k
```
...
X <- scale(data.matrix(diamonds[x])) #theoretically you should scale the folds seperately
...
for (i in 1:nrow(paramGrid)) {
  ...
  paramGrid[i, "RMSE"] <- mean(scores) #stability by taking the k-th root
}
...
- unfair a bit, b/c we used 20 different nearest neighbors, but just 1 linear model.
- classic way is to validate a model by p-values, but for these kind of models like nearest neighbors you don't have them.
...
duckdb_register(con, name = "dia", df = diamonds) #register data into database (not written into it)

# Select every column (and every row)
con %>% 
  dbSendQuery("SELECT * FROM dia") %>% 
  dbFetch() %>% #pass the output of the query back to R
  head()  # See note above
...
query <- "
  ...
  WHERE 
    carat > 2 
    AND color IN ('E', 'D')
" #no double quotes, just single quotes
con %>% 
  dbSendQuery(query) %>% 
  dbFetch() %>% 
  head()
...
query <- "
  ...
  HAVING N >= 100000
" #HAVING is like WHERE but after aggregation
con %>% 
  dbSendQuery(query) %>% 
  dbFetch() %>% 
  head()
