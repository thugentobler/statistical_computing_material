...
Has the same Problem as the trees, cant extrapolate
...
cb <- list(
  ...
  callback_reduce_lr_on_plateau(patience = 5) #reduce the learning rate if the loss is on a plateau
)
...
fl <- flashlight(
  ...
  predict_function = function(m, X) 
    predict(m, prep_nn(X), batch_size = 1000, verbose = 0) #need to scale X here as well
)
