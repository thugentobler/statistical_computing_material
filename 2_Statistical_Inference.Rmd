...
#same logic for every possible statistic
boot <- replicate(2000, median(sample(x, replace = TRUE)))
...
- but *more Bootstrap samples* are usually required to get stable results (why?). (like 10000)
...
- "BCa": Bias-corrected and accelerated. In theory the most accurate of the four, and thus in practice often the recommended type of interval. Improved from the percentile method
...
estimator <- function(X) {
  cor(X)[1, 2] #cor(x) correlation matrix
}
(est <- estimator(df))
...
boot <- replicate(
  9999, 
  estimator(df[sample(nrow(df), replace = TRUE), ]) #resampling rows of dataset
)#table(sample(25, replace= True)) gives 
...
# Standard normal Bootstrap CI
est + c(-1, 1) * qnorm(0.975) * sd(boot) #upper limit could be bigger than one

# Percentile Bootstrap CI
quantile(boot, probs = c(0.025, 0.975)) #always between zero and one
```
...
m <- 20 #not paired situation, different sample sizes
n <- 40
...
#standardised mean distance -2.3297
# Calculate p value "by hand" (why multiply with 2? two sided test)
...
statistic <- function(x, y) {
  abs(mean(y[x == "B"]) - mean(y[x == "A"])) #easier to calculate p-value if abs()
}

# Calculate approximate null distribution
B <- 1e4
perm_dist <- replicate(B, statistic(sample(x), y)) #sample can permute

# p value
mean(perm_dist >= statistic(x, y)) #relative count of cases that are equal or bigger than statistic
...
- Power: Probability to correctly reject wrong $H_0$ (for specific given alternative). 1 minus Type 2 error. Depend on something
...
if (FALSE) {  # Set to TRUE to rerun
  for (i in 1:nsim) {
    ...
    result[i, "Welch"] <- t.test(y ~ x)$p.value <= 0.05 #is the p value small?
...
(Power <- readRDS("simulation/Power.rds")) #want to maximise, not to a specific value
```
...
# Permutation test
statistic <- function(x, y) {
  r <- rank(y) #replace y values with their ranks
  abs(mean(r[x == "B"]) - mean(r[x == "A"]))
  # Could also use Wilcoxon's rank-sum statistic
}

with_seed( #changes seed only in this brackets
  38,
  perm_dist <- replicate(1e4, statistic(sample(x), y)) #resample one column, not the rows (that would be bootstrap)
)
