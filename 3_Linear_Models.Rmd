...
ggplot(diamonds, aes(x = carat, y = price)) + 
  ...
  geom_abline(slope = slope, intercept = intercept, color = "chartreuse4", linewidth= 1)
...
(RMSE <- sqrt(MSE)) #interpretation: typical error of the model is around this value
...
# R-squared
(MSE_empty - MSE) / MSE_empty #percent can be explained by the model
```

Is the model correct? no, its just a approximation.
Is the model sufficiently close? no.
...
Estimates of factor is respect to colorD, the number gives the increase of the price given the color changes from D to the factor.
R have no logical datatype, therefore every factor is a integer (very inefficient for large number of factors)
...
If the carat goes up by 1%, the price goes up by `r fit$coefficients[2]/100` percent.
...
When the carat goes up by 0.1, then the price goes up by `r fit$coefficients[2]*10` percent.
Strange behavior of the model above carat = 2.
...
#bias because of the logarithm and the Expectation, which depends on assumptions which are wrong.
```

R-squared goes up, but its not fair, since the outliers goes down. Good behavior on the original scale.
How to interpret? for the original scale, interpret the data as percent, if carat by 1 percent, the price goes up by `r fit$coefficients[2]` percent.
...
The elasicity went up to `r fit$coefficients[2]`. The color switch goes down monotonically. The better the cut, the higher the price. Clarity from the worst to the best makes sense.
...
# Bias on original scale? the loglink is the natural link of the poisson family, all other link functions will give a bias
...
setDT(df)
head(df)
# fwrite(df, file = "taxi/test.csv") # 767 MB vs. 117 MB Parquet #fastest csv loader in R
