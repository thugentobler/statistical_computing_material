...
fit <- rpart(
  clm ~ veh_value + veh_body + veh_age + gender + area + agecat, #claims 
  data = dataCar,
  method = "class", 
  parms = list(split = "information"), #"information" <-> binary cross-entropy
  xval = 0,
  cp = -1,
  maxdepth = 3
)
#modern algorithms bin the input space to make the fitting faster
...
#a monotonic transformation on a feature in a decision tree does not change the outcome, since the ranks wont be changed. but if it is binned, this might have an impact
...
fit_unbagged <- rpart(form, data = train, xval = 0, cp = -1, maxdepth = 8) #relativly deep maxdepth = 8
...
round(cor(valid_pred[, 1:3]), 3) #there is a benefit of the not full correlation of the models
...
1. Select $B$ bootstrapped training data sets from the original training data.
2. Fit a (usually deep) decision tree $\hat f^{*j}(\boldsymbol x)$ on each of them. During each split, consider only a random subset of features.
3. Return the model 
  $$
    \hat f(\boldsymbol x) = \frac{1}{B} \sum_{j = 1}^B f^{*j}(\boldsymbol x).
  $$
...
fit <- ranger(
  reformulate(x, y), 
  num.trees = 500, #very big, 
  data = diamonds[ix, ], 
  importance = "impurity",
  seed = 83
)
fit
#does OOB prediction error and R squared

# Performance on test data
pred <- predict(fit, diamonds[-ix, ])$predictions
y_test <- diamonds[-ix, y]
rmse(y_test, pred)
r_squared(y_test, pred) #~10\% is 2\% is better, but not that much. normaly it is truly better with more trees

# Size of the fitted model
format(object.size(fit), units = "auto") #xgboost models are just around 1mb
```
...
imp <- imp / sum(imp)
barplot(imp, horiz = TRUE, col = "chartreuse4") #log_carat: golden feature (decides almost everything)
```
...
light_ice(fl, v = v, n_max = 200, center = "middle") %>% 
  ...
  labs(title = paste("c-ICE plot for", v), y = "Centered prediction") #shifted at a point, remove the vertical spread, tail off effect -> tree based models cannot extrapolate
...
light_ice(fl, v = v, n_max = 200, center = "middle") %>% 
  plot(alpha = 0.1, color = "chartreuse4") +
  labs(title = paste("c-ICE plot for", v), y = "Centered prediction") #strong interaction effects
```
...
The solution is to replace $-g_i$ by predictions of a decision tree (or of another base learner) with covariates $\boldsymbol X$. This leads us to the following algorithm:
...
dtrain <- xgb.DMatrix(data.matrix(X_train), label = y_train) #careful with character values, for prediction the integer encoding needs to be the same! F = 1, M = 2 at training, at prediction, M = 1 -> wrong answer
...
if (tune) {
  ...
  # to figure out how many rounds we need -> a,round 200, between 100 and 1000, thats ok
  ...
  grid <- expand.grid(
    ...
    cv_score = NA,
    train_score = NA,
    learning_rate = 0.1, #now fixed to 0.1
    objective = "reg:squarederror",
    eval_metric = "rmse",
    max_depth = 6:7, 
    min_child_weight = c(1, 10),
    colsample_bytree = c(0.8, 1), #not use all columns in each tree
    subsample = c(0.8, 1), #not use all rows in each tree
...
rmse(y_test, pred)
r_squared(y_test, pred, reference_mean = mean(y_train)) #never see such high values in practice
...
if (tune) {
  ...
  params <- list(
    learning_rate = 1, #learning rate of 1 seems to makes sense here, b/c of the dataset
    objective = "mse",
