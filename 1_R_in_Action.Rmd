...
p <- ggplot(diamonds, mapping = aes(x = color, y = price)) +
  geom_boxplot(fill = "chartreuse4", varwidth = TRUE) #varwidth shows widht corresponding to number of points
p
...
p + coord_cartesian(ylim = c(0, 8000)) #functions to change the coordinate system
...
diamonds %>% 
  select(price, carat) %>% 
  pivot_longer(everything()) %>% ###need that to do faceting!!
ggplot(aes(value)) +
...
gcd <- function(a, b) {
  stopifnot(
    "a should be a single number" = length(a) == 1, #outputs a warning message
    length(b) == 1,
    a >= 1,
    b >= 1,
    a == trunc(a),
...
# the plot function looks at the class of the object price~color and then calls the function plot.formula
class(price ~ color)
